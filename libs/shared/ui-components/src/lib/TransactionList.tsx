...
import { sortAccounts } from '@useful-apps/shared/utils';
...
export const TransactionList: React.FC<TransactionListProps> = ({
  ...
  const filteredTransactions = useMemo(() => {
    return transactions
      .filter(t => {
        ...
        if (filterPayee && !p.toLowerCase().includes(filterPayee.toLowerCase())) return false;
  ...
  // Opening balance: account's initialBalance + every transaction BEFORE the filter start date.
  // This lets us compute a closing balance that matches the liquidity graph.
  const openingBalance = useMemo(() => {
    ...
    transactions.forEach(t => {
      // Only replay history that is strictly before the visible date window
      if (filterDateFrom && t.date >= filterDateFrom) return;
      if (t.type === TransactionType.INCOME && t.accountId === activeAccountId) {
        ...
        if (t.accountId === activeAccountId)   balance -= t.amount; // outflow
        if (t.toAccountId === activeAccountId) balance += t.amount; // inflow
      }
    });
    return balance;
  }, [activeAccountId, accounts, transactions, filterDateFrom]);
      ...
      <div className="p-6 border-b border-gray-100 bg-white shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          {filteredTransactions.length > 0 && (
            <div className="flex items-stretch gap-0 rounded-xl overflow-hidden border border-gray-100 shadow-sm shrink-0">
              {/* Opening balance pill — single-account view only */}
              {activeAccountId && openingBalance !== null && (
                <>
                  <div className="flex flex-col items-center px-5 py-2.5 bg-slate-100">
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">
                      {filterDateFrom ? `Balance ${filterDateFrom}` : 'Opening'}
                    </span>
                    <span className="text-sm font-black text-slate-700">{formatCurrency(openingBalance, getCurrency(activeAccountId))}</span>
                  </div>
              ...
              <div className="w-px bg-gray-100" />
              {/* In single-account view: show closing balance = opening + net change.
                  In consolidated view: show simple net of the period. */}
              {activeAccountId && openingBalance !== null ? (
                <div className={`flex flex-col items-center px-5 py-2.5 ${
                  openingBalance + filteredSummary.net >= 0 ? 'bg-brand-50' : 'bg-orange-50'
                }`}>
                  <span className={`text-[8px] font-black uppercase tracking-widest mb-0.5 ${
                    openingBalance + filteredSummary.net >= 0 ? 'text-brand-500' : 'text-orange-500'
                  }`}>Balance Now</span>
                  <span className={`text-sm font-black ${
                    openingBalance + filteredSummary.net >= 0 ? 'text-brand-700' : 'text-orange-600'
                  }`}>{formatCurrency(openingBalance + filteredSummary.net, getCurrency(activeAccountId))}</span>
                </div>
              ) : (
        ...
        {showFilters && (
          <div className="mt-4 p-4 bg-gray-50 border border-gray-100 rounded-xl grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 animate-fade-in shadow-inner">
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Payee Name</label>
              <input type="text" value={filterPayee} onChange={e => setFilterPayee(e.target.value)} placeholder="Filter by payee..." className="w-full p-2 border border-gray-200 rounded-lg text-xs bg-white" />
            </div>
            <div className="space-y-1">
              <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest ml-1">Min Amount</label>
              <input type="number" value={filterMinAmount} onChange={e => setFilterMinAmount(e.target.value)} placeholder="0.00" className="w-full p-2 border border-gray-200 rounded-lg text-xs bg-white" />
            </div>
            <div className="flex items-end">
              <button
                onClick={() => { setFilterDateFrom(''); setFilterDateTo(''); setFilterCategory(''); setFilterAccount(''); setFilterMinAmount(''); setFilterMaxAmount(''); setFilterPayee(''); }}
